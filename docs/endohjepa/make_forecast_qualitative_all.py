...
def main():
    ...
    persistence_cos = F.cosine_similarity(
        persistence[:, -1], future[:, -1], dim=-1)
    ...
    for name in sorted(by_dataset):
        ...
        def _retrieve(query: torch.Tensor) -> tuple[int, str]:
            if len(cross_clip):
                position = int(torch.cdist(
                    query[None], future[cross_clip, -1]).argmin())
        ...
        example_cos_model = float(F.cosine_similarity(
            pred_terminal, target_terminal, dim=0))
        example_cos_persistence = float(F.cosine_similarity(
            persistence_terminal, target_terminal, dim=0))
        rows.append({
            "dataset": name,
        ...
        examples.append((name, chosen, retrieved_index, persistence_retrieved_index,
                         retrieval_scope, example_cos_model, example_cos_persistence))
    ...
    titles = [
        ...
        "Forecast-latent retrieval", "Persistence-latent retrieval",
    ]
    for part, chunk in enumerate((examples[:6], examples[6:]), start=1):
        ...
        for row_index, example in enumerate(chunk):
            (name, index, retrieved_index, persistence_index, scope,
             cos_model, cos_persistence) = example
            ...
            persistence_image = _read(
                clips[persistence_index].frame_paths()[terminal])
            ...
            for column, (label, colour) in enumerate((
                    ...
                    (f"cos {cos_persistence:.3f}", "#8A5A2F")), start=2):
