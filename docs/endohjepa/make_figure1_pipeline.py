...
* four strict rows (three passive-video orifices plus the pose-gated physical
...
BRANCH_Y = 3.50
MOD_H_A = 3.56          # tall lane-A modules span all three orifice rows
MOD_H_B = 1.90
GATE_Y = 0.38
...
def thumb(ax, name, slot_cx, cy, ec, label, sub):
    ...
    ax.text(slot_cx, y0 - 0.10, label, ha="center", va="top", fontsize=8.2,
            fontweight="bold", color=INK)
    ax.text(slot_cx, y0 - 0.33, sub, ha="center", va="top", fontsize=7.0,
            color=MUTED, style="italic")
    return x0, x0 + w


def frame(ax, left, right, cy, h, fc, ec, lw=1.2):
    ax.add_patch(FancyBboxPatch((left, cy - h / 2), right - left, h,
...
def module(ax, left, right, cy, title, lines, fc, ec, glyph, h=MOD_H_B):
    ...
    ax.text(cx, cy + h / 2 - 0.18, title, ha="center", va="top",
            fontsize=9.0, fontweight="bold", color=INK, zorder=6)
    glyph(ax, cx, cy + 0.20)
    ax.text(cx, cy - h / 2 + 0.16, lines, ha="center", va="bottom",
            fontsize=7.2, color=MUTED, linespacing=1.45, zorder=6)


def subbox(ax, left, right, top, h, title, lines, ec):
    """Nested block inside a tall module."""
    ax.add_patch(FancyBboxPatch((left, top - h), right - left, h,
                                boxstyle="round,pad=0,rounding_size=0.04",
                                linewidth=0.9, edgecolor=ec, facecolor=WHITE,
                                zorder=5))
    cx = (left + right) / 2
    ax.text(cx, top - 0.17, title, ha="center", va="top", fontsize=7.8,
            fontweight="bold", color=INK, zorder=6)
    ax.text(cx, top - h + 0.12, lines, ha="center", va="bottom", fontsize=6.9,
            color=MUTED, linespacing=1.40, zorder=6)


def harrow(ax, x1, x2, y, label=None):
    ax.plot([x1, x2 - 0.05], [y, y], color=LINE, lw=1.25, zorder=Z_WIRE,
            solid_capstyle="butt")
    ax.annotate("", xy=(x2, y), xytext=(x2 - 0.05, y),
                arrowprops=dict(arrowstyle="-|>", color=LINE, lw=1.25,
                                mutation_scale=10), zorder=Z_WIRE)
    if label:
        ax.text((x1 + x2) / 2, y + 0.09, label, ha="center", va="bottom",
...
def varrow(ax, x, y1, y2):
    ...
    ax.plot([x, x], [y1, y2 - step], color=LINE, lw=1.25, zorder=Z_WIRE,
            solid_capstyle="butt")
    ax.annotate("", xy=(x, y2), xytext=(x, y2 - step),
                arrowprops=dict(arrowstyle="-|>", color=LINE, lw=1.25,
                                mutation_scale=10), zorder=Z_WIRE)
...
def route_hvh(ax, x1, y1, x2, y2, xm):
    ...
    ax.plot([x1, xm], [y1, y1], color=LINE, lw=1.25, zorder=Z_WIRE,
            solid_capstyle="butt")
    if abs(y2 - y1) > 1e-6:
        ax.plot([xm, xm], [y1, y2], color=LINE, lw=1.25, zorder=Z_WIRE,
                solid_capstyle="butt")
    harrow(ax, xm, x2, y2)
...
def route_vhv(ax, x1, y1, ym, x2, y2):
    ...
    ax.plot([x1, x1], [y1, ym], color=LINE, lw=1.25, zorder=Z_WIRE,
            solid_capstyle="butt")
    ax.plot([x1, x2], [ym, ym], color=LINE, lw=1.25, zorder=Z_WIRE,
            solid_capstyle="butt")
    varrow(ax, x2, ym, y2)
    ...
    for r in range(rows):
        for c in range(cols):
            ax.add_patch(Rectangle(
                ...
                cw, ch, facecolor="#DCE9F3", edgecolor=C_ENC, lw=0.7, zorder=7))
...
def glyph_forecast(ax, cx, cy, w=0.98, h=0.42):
    ...
    ax.plot([x, x + w * 0.42], [y + h * 0.28, y + h * 0.28], color=INK, lw=1.3,
            zorder=6)
    ax.scatter(x + w * np.linspace(0.04, 0.38, 4), np.full(4, y + h * 0.28),
               s=7, color=INK, zorder=7)
    ax.plot(x + w * (0.42 + 0.58 * t), y + h * (0.28 + 0.64 * t), color=C_FC,
            lw=1.5, zorder=6)
    ax.plot([x + w * 0.42, x + w], [y + h * 0.28, y + h * 0.28], color=LINE,
            lw=1.0, ls="--", zorder=6)
...
def glyph_se3(ax, cx, cy, s=0.40):
    ...
    ax.annotate("", xy=(x0 + s, y0), xytext=(x0, y0),
                arrowprops=dict(arrowstyle="-|>", color="#B84A4A", lw=1.5), zorder=6)
    ax.annotate("", xy=(x0, y0 + s * 0.80), xytext=(x0, y0),
                arrowprops=dict(arrowstyle="-|>", color="#3F7A5A", lw=1.5), zorder=6)
    ax.annotate("", xy=(x0 - s * 0.46, y0 - s * 0.34), xytext=(x0, y0),
                arrowprops=dict(arrowstyle="-|>", color=C_ENC, lw=1.5), zorder=6)
...
def scaling_inset(ax, left, right, bottom, top):
    ...
    ax.plot([left, right], [baseline, baseline], color="#B0888A", lw=1.0,
            ls="--", zorder=6)
    ...
    ax.scatter(px[-1:], py[-1:], s=11, facecolor=WHITE, edgecolor=C_FC,
               linewidths=0.9, zorder=7)
    ax.text(right, baseline - 0.04, f"persistence {PERSISTENCE_COS:.3f}", ha="right", va="top",
            fontsize=5.9, color="#9A6C6E", zorder=6)
    ax.text(right, top + 0.01, f"{SCALE_COS[-1]:.3f}", ha="right", va="bottom", fontsize=5.9,
            color=C_FC, zorder=6)
    ax.text(left - 0.04, (bottom + top) / 2, "cosine", ha="center", va="center",
            fontsize=5.9, color=MUTED, rotation=90, zorder=6)
    ax.text((left + right) / 2, bottom - 0.06, "training clips 0.5k $\\to$ 13.5k",
            ha="center", va="top", fontsize=5.9, color=MUTED, zorder=6)
...
def hierarchical_predictor(ax, cy):
    ...
    ax.text(cx, top - 0.18, "Hierarchical predictor", ha="center", va="top",
            fontsize=9.0, fontweight="bold", color=INK, zorder=6)
    ax.text(cx, top - 0.42, "two temporal scales, encoder space only",
            ha="center", va="top", fontsize=7.0, color=MUTED, style="italic",
            zorder=6)
    inner_l, inner_r = MOD_L + 0.20, MOD_R - 0.20
    subbox(ax, inner_l, inner_r, top - 0.62, 1.12, "L1: pooled causal residual",
           "GPT-style causal transformer\nhorizon 4 tubelets\n"
           "frozen-teacher targets", C_FC)
    subbox(ax, inner_l, inner_r, top - 2.08, 1.12, "L2: pooled coarse residual",
           "temporally pooled tokens\nmid-horizon abstraction\n"
           "domain embedding $e_d$", C_FC)
    varrow(ax, cx, top - 1.74, top - 1.96)
    ax.text(cx + 0.10, top - 1.85, "pooled state", ha="left", va="center",
            fontsize=6.4, color=MUTED, style="italic", zorder=9)
    ax.text(cx, cy - MOD_H_A / 2 + 0.09,
            "cosine target; no pixel decoder",
            ha="center", va="bottom", fontsize=6.6, color=C_FC, zorder=6,
            linespacing=1.35)
...
def forecasting_capability(ax, cy):
    ...
    bottom = cy - MOD_H_A / 2
    ax.text(cx, top - 0.18, "Capability 1: forecasting", ha="center", va="top",
            fontsize=9.0, fontweight="bold", color=INK, zorder=6)
    ax.text(cx, top - 0.42, "predict future encoder states",
            ha="center", va="top", fontsize=7.0, color=MUTED, style="italic",
            zorder=6)
    scaling_inset(ax, CAP_L + 0.55, CAP_R - 0.30, top - 1.98, top - 0.70)
    ax.text(cx, bottom + 0.16,
...
def shared_encoder(ax):
    ...
    frame(ax, ENC_L, ENC_R, (ENC_TOP + ENC_BOT) / 2, ENC_TOP - ENC_BOT,
          SOFT_ENC, C_ENC)
    cx = (ENC_L + ENC_R) / 2
    ax.text(cx, ENC_TOP - 0.18, "Shared encoder", ha="center", va="top",
            fontsize=9.0, fontweight="bold", color=INK, zorder=6)
    ax.text(cx, ENC_TOP - 0.44, "frozen V-JEPA 2 ViT-L", ha="center", va="top",
            fontsize=7.3, color=MUTED, style="italic", zorder=6)
    ...
    stages = [
        ...
        (4.82, 0.18, lambda x, y: token_grid(ax, x, y),
         "spatially pooled tokens $\\bar z$, $8{\\times}1024$"),
    ]
    for index, (cy, half, draw, caption) in enumerate(stages):
        draw(cx, cy)
        ax.text(cx, cy - half - 0.05, caption, ha="center", va="top",
                fontsize=7.0, color=MUTED, zorder=6)
        if index + 1 < len(stages):
            next_cy, next_half = stages[index + 1][0], stages[index + 1][1]
            varrow(ax, cx, cy - half - 0.30, next_cy + next_half + 0.08)
    ax.text(cx, ENC_BOT + 0.10,
            "$+$ domain embedding $e_d$\none encoder for all three orifices",
            ha="center", va="bottom", fontsize=6.9, color=C_ENC, zorder=6,
            linespacing=1.35)
...
def main():
    ...
    module(ax, ENC_L, ENC_R, ROW_PHYS, "Tubelet-aligned SE(3) actions",
           "$u_t=\\log(T_t^{-1}T_{t+1})$, camera frame\n"
           "one twist per 2-frame tubelet\ndepth gives near-wall risk labels",
           SOFT_PHYS, C_PHYS, glyph_se3)
    module(ax, MOD_L, MOD_R, ROW_PHYS, "SE(3) latent dynamics",
           "block-causal action path\nprobabilistic ensemble, $\\mu,\\Sigma$\n"
           "risk head unused (AUC 0.523 fail)",
           SOFT_PHYS, C_PHYS, glyph_forecast)
    module(ax, CAP_L, CAP_R, ROW_PHYS, "SE(3)-conditioned evaluation",
           f"batch-shuffled action win "
           f"{100 * ACTION_AUDIT['batch_shuffled']['real_action_win_fraction']:.1f} % "
           f"($n={ACTION_AUDIT['n_windows']}$)\n"
           f"oracle-goal proxy win {100 * NAV_PROXY['proxy_win_fraction']:.1f} % "
           f"({NAV_PROXY['n_trials']} windows)\n"
           f"retrieved-pose error "
           f"$-{100 * NAV_PROXY['retrieval_translation_error_reduction_fraction']:.1f}\\,\\%$ "
           "vs persistence",
           WHITE, C_PHYS, glyph_navigation)
    ...
    for cy, src, dst, app in zip(ROW_FC, ROWS_IN, ROWS_OUT, APPS):
        ...
        ax.text((RISER + out_left) / 2, cy + 0.11, app, ha="center", va="bottom",
                fontsize=6.5, color=C_APP, style="italic", linespacing=1.3)

    harrow(ax, ENC_R, MOD_L, mid_fc, label="tokens $z$")
    harrow(ax, MOD_R, CAP_L, mid_fc, label="forecast $\\hat{z}$")

    # ---- pose-gated physical row -------------------------------------------
    _, in_right = thumb(ax, "_fig1_in_scared.png", in_cx, ROW_PHYS, C_PHYS,
                        "SCARED (laparoscopy)", "frame + calibrated pose")
    out_left, _ = thumb(ax, "_fig1_se3.png", res_cx, ROW_PHYS, C_PHYS,
                        "SCARED (laparoscopy)", "SE(3)-conditioned retrieval")
    ...
    ax.text(mod_cx - 0.10, BRANCH_Y + 0.09, "same frozen tokens", ha="right",
            va="bottom", fontsize=6.8, color=MUTED, style="italic")
    ...
    for index, gate in enumerate(gates):
        ...
        ax.add_patch(FancyBboxPatch((left, GATE_Y - 0.32), width, 0.64,
                                    boxstyle="round,pad=0,rounding_size=0.05",
                                    linewidth=1.0, edgecolor=C_AUD,
                                    facecolor=SOFT_AUD, zorder=2))
        ax.text(left + width / 2, GATE_Y, gate, ha="center", va="center",
                fontsize=7.2, color=INK, linespacing=1.4, zorder=6)
    ...
    fig.savefig(OUT / "figure1_pipeline.png", dpi=220, bbox_inches="tight",
                facecolor="white")
