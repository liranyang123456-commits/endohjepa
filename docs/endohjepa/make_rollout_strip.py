...
def main():
    ...
    summary.plot(steps, steps, "-o", color="#3F7A5A", lw=1.8, ms=5,
                 label="ground truth")
    summary.errorbar(steps, mean_model, yerr=standard_error, fmt="-o",
                     color="#2F5D8A", lw=1.8, ms=5, capsize=3,
                     label="model rollout")
    ...
    summary.set_title(
        f"Aggregate over {len(records)} held-out windows", fontsize=9.8,
        fontweight="bold")
