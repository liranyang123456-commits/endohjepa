...
def _thumb(ax, path: Path, cx: float, cy: float, w: float, h: float):
    ...
    ax.imshow(array, extent=(left, left + draw_w, bottom, bottom + draw_h),
              zorder=3)
    ax.add_patch(Rectangle(
        (left, bottom), draw_w, draw_h, facecolor="none",
...
def main() -> None:
    ...
    thumbnails = [
        ("_fig1_in_laparo.png", "Laparoscopy"),
        ("_fig1_in_gi.png", "GI endoscopy"),
        ("_fig1_in_bronch.png", "Bronchoscopy"),
    ]
    ...
    ax.text(0.04, 0.95, "Endo-HJEPA", fontsize=18, fontweight="bold",
            color=INK, va="top")
    ax.text(
        0.04, 0.875,
    ...
    _box(ax, 0.04, 0.28, 0.53, 0.49, SOFT_ORANGE, ORANGE,
         "Observed endoscopic video")
    thumb_y = 0.54
    for index, (name, label) in enumerate(thumbnails):
        cx = 0.14 + 0.165 * index
        _thumb(ax, OUT / name, cx, thumb_y, 0.145, 0.19)
        ax.text(cx, 0.405, label, ha="center", va="top", fontsize=8.5,
                fontweight="bold", color=INK)
    ax.text(0.305, 0.325, "real frames · no pixel synthesis",
            ha="center", va="center", fontsize=9, color=MUTED)

    _box(ax, 0.69, 0.28, 0.45, 0.49, SOFT_BLUE, BLUE,
    ...
    for row in range(3):
        for column in range(7):
            ax.add_patch(Rectangle(
                ...
                0.032, 0.028, facecolor="#C9DDEA", edgecolor=BLUE,
                linewidth=0.65, zorder=4))
    ax.text(0.915, 0.445, "pooled spatiotemporal tokens",
            fontsize=9, color=BLUE, ha="center", fontweight="bold")
    _arrow(ax, (0.58, 0.525), (0.68, 0.525), BLUE)

    _box(ax, 1.29, 0.52, 0.52, 0.25, SOFT_TEAL, TEAL,
         "Forecast future representations",
         "validated L1 residual forecast;\nL2 implemented, not ablated")
    ax.add_patch(FancyBboxPatch(
        (1.37, 0.585), 0.15, 0.055,
        boxstyle="round,pad=0.008", facecolor="white", edgecolor=TEAL,
        linewidth=1.0, zorder=4))
    ax.add_patch(FancyBboxPatch(
        (1.58, 0.585), 0.15, 0.055,
        boxstyle="round,pad=0.008", facecolor="white", edgecolor=TEAL,
        linewidth=1.0, zorder=4))
    ax.text(1.445, 0.612, "L1", fontsize=9, fontweight="bold",
            ha="center", va="center", color=INK, zorder=5)
    ax.text(1.655, 0.612, "L2", fontsize=9, fontweight="bold",
            ha="center", va="center", color=INK, zorder=5)
    _arrow(ax, (1.15, 0.60), (1.28, 0.645), TEAL)

    _box(ax, 1.29, 0.28, 0.52, 0.18, SOFT_PURPLE, PURPLE,
         "Audited physical association",
         "deterministic action dynamics → CEM retrieval proxy")
    _arrow(ax, (1.15, 0.43), (1.28, 0.37), PURPLE)

    _box(ax, 1.94, 0.52, 0.50, 0.25, "#F7FAF8", TEAL,
         "Latent forecast evidence",
         "h=4 cosine 0.978 vs 0.916 persistence\nretrieval visualisation, not generation")
    _arrow(ax, (1.82, 0.645), (1.93, 0.645), TEAL)

    _box(ax, 1.94, 0.28, 0.50, 0.18, "#F8F5FA", PURPLE,
         "Oracle-goal retrieval proxy",
         "200 windows · 60% win · not robot control")
    _arrow(ax, (1.82, 0.37), (1.93, 0.37), PURPLE)

    _box(ax, 1.94, 0.09, 0.50, 0.11, "#FAFAFA", "#888888",
         "Separate risk checkpoint: inactive", dashed=True)
    ax.text(2.19, 0.115, "AUC 0.523 < 0.75 gate", fontsize=8.5,
            color=MUTED, ha="center", va="center", zorder=5)
    ...
    provenance = {
        "figure": ["figures/graphical_abstract.pdf",
    ...
    print(
        f"[graphical abstract] wrote {pixel_size[0]}x{pixel_size[1]} "
        f"at {DPI} dpi")
