...
- Continuous SE(3) action sensitivity (v2 corrected pipeline): **85.2%**
  batch-shuffled real-action win on the audit-selected held-out SCARED
  partition (958 overlapping windows, descriptive interval; grouped-CV macro
  88.9%); external C3VD ten-trajectory pooled 58.3% (above chance, below
  gate).
- Offline navigation proxy (single-shot oracle-goal latent retrieval, 200
  trials): **60.0%** of retrievals beat persistence [53.0, 66.5], 33.8%
  translation-error reduction [26.8, 39.9]. This is an offline proxy, not
  receding-horizon control or robot execution.
- Calibrated near-wall risk head: test AUC **0.523** — fails the 0.75 gate;
  reported as a negative result, not used for filtering.
