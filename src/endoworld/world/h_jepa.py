...
class HJEPAConfig:
    ...
    residual: bool = True         # predict delta from the last token (persistence + correction)
    l1_causal: bool = True        # autoregressive causal L1 (GPT-style); beats query-token & GRU
...
class TransformerPredictor(nn.Module):
    ...
    def __init__(self, cfg: HJEPAConfig, action_cond: bool = False, horizon: int | None = None):
        super().__init__()
        self.cfg = cfg
    ...
    def forward(self, z_hist, domain_tok=None, actions=None):
        ...
        seq = seq + self._positions(
            seq.size(1), seq.size(2), seq.device, seq.dtype)
        seq = self.encoder(
            seq,
            mask=self._block_causal_mask(
                z_hist.size(1), self.horizon, seq.device),
        )
...
class SpatioTemporalBlock(nn.Module):
    ...
    def __init__(self, dim: int, n_heads: int, dropout: float):
        ...
        self.temporal = nn.TransformerEncoderLayer(
            dim, n_heads, dim * 4, dropout, batch_first=True,
...
class SpatioTemporalPredictor(nn.Module):
    ...
    def __init__(self, cfg: HJEPAConfig, max_t: int = 64, max_n: int = 1024):
        ...
        self.blocks = nn.ModuleList(
            [SpatioTemporalBlock(h, cfg.n_heads, cfg.dropout) for _ in range(cfg.n_layers)])
...
class CausalTemporalPredictor(nn.Module):
    ...
    def __init__(self, cfg: HJEPAConfig, max_t: int = 64):
        ...
        layer = nn.TransformerEncoderLayer(
            h, cfg.n_heads, h * 4, cfg.dropout, batch_first=True,
            activation="gelu", norm_first=True)
...
class EndoHJEPA(nn.Module):
    def __init__(self, cfg: HJEPAConfig):
        ...
        self.l1 = CausalTemporalPredictor(cfg) if getattr(cfg, "l1_causal", False) \
            else TransformerPredictor(cfg, action_cond=False)
        self.l1_dense = SpatioTemporalPredictor(cfg) if cfg.predictor == "spacetime" else None
    ...
    def losses(self, z, domain_id, history: int, horizon: int):
        ...
        out = {"l1": F.smooth_l1_loss(pred1[:, :h1], z_future[:, :h1]),
               "pred": pred1}
        mode = self.cfg.ablation
        if mode in ("l1l2", "full"):
            z_l2_tgt = z_future[:, ::self.cfg.l2_stride]
        ...
        if mode == "full":
            ...
            pred3 = self.forward_l3(z_hist, act_idx[:, history - 1:history - 1 + horizon], domain_id)
        ...
        if "l3" in out:
            total = total + self._uw("l3", out["l3"]) \
                + self._uw("energy", out["energy"]) + self._uw("commit", out["commit"])
        out["total"] = total
        return out
    ...
    def plan(self, z_hist, z_goal, domain_id, n_samples: int = 32, steps: int = 4):
        return latent_mpc(self, z_hist, z_goal, domain_id, n_samples=n_samples, steps=steps)
