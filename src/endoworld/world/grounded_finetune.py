...
def grounding_metric(model, Z0, Z1, V, vids, device):
    ...
    tr = [
        ...
        if assign_split(f"t50::{vv}", train=0.8, val=0.0) == "train"
    ]
    te = [
        i
        for i, vv in enumerate(vids)
...
def main():
    ...
    data = build_transitions(
        enc, device, args.root, args.clip_len, args.max_per_video, args.max_videos
    )
    if data is None:
    ...
    torch.save(
        {"actions": model.actions.state_dict(), "verb_head": verb_head.state_dict()},
