...
@torch.no_grad()
def encode_dense_past_only(enc, frames: torch.Tensor, device: str,
                           lookback_frames: int, tubelet: int, batch: int = 4):
...
def main():
    ...
    for keyframe in find_scared_keyframes(args.scared):
        ...
        cap.release()
        indices = np.arange(0, min(total, len(poses)), args.stride, dtype=np.int64)[:args.max_frames]
        frames = read_video_frames(video, indices, enc.image_size, stereo_eye="top")
        dense = encode_dense_past_only(
            enc, frames, device, lookback_frames, tubelet)
    ...
    torch.save({"rows": rows, "lookback_tubelets": args.lookback_tubelets,
                "stride": args.stride, "tubelet": tubelet}, out)
    meta = {"n_sequences": len(rows),
