...
def load_encoder(args, device):
    ...
    cfg = VJEPAConfig(image_size=96, clip_len=16, embed_dim=256, depth=4, num_heads=4,
                      predictor_dim=128, predictor_depth=2)
    model = VJEPA(cfg).to(device).eval()
    for p in model.parameters():
        p.requires_grad_(False)
    return model, cfg.clip_len, cfg.image_size, cfg.embed_dim
...
def cache_latents(enc, dl, device, dense: bool):
    ...
    with torch.no_grad():
        for bi, (clip, domain) in enumerate(dl):
            ...
            Doms.append(domain)
            if (bi + 1) % 10 == 0:
                print(f"  encoded {sum(z.shape[0] for z in Z)} ({time.time()-t0:.0f}s)")
...
def train(args):
    ...
    if args.smoke:
        from endoworld.understanding.vjepa import VJEPA, VJEPAConfig
        vcfg = VJEPAConfig(image_size=64, clip_len=8, embed_dim=128, depth=2,
    ...
    if args.clip_len:
        clip_len = args.clip_len  # override to control cache memory (T = clip_len / tubelet)
    ...
    if reuse:
        ...
        print(f"[data] train clips={Z.size(0)} (from cache) dense={args.dense} ablation={args.ablation}")
    else:
        ds = EndoClipDataset(
            args.manifest, clip_len=clip_len, stride=args.stride,
            image_size=image_size, exclude=["EndoVis2019_ROBUST-MIS"],
            return_meta=True, split=split_tr,
            include_domains=args.domains.split(",") if args.domains else None,
        )
        _take_clips(ds, args)
        print(f"[data] train clips={len(ds)} split={split_tr or 'all'} dense={args.dense} ablation={args.ablation}")
        dl = DataLoader(ds, batch_size=args.batch_size, shuffle=True,
                        num_workers=args.workers, collate_fn=collate_meta, drop_last=True)
    ...
    if reuse and pack is not None and pack.get("Z_val") is not None:
        ...
        ds_va = EndoClipDataset(
            args.manifest, clip_len=clip_len, stride=args.stride,
            image_size=image_size, exclude=["EndoVis2019_ROBUST-MIS"],
            return_meta=True, split=split_va,
            include_domains=args.domains.split(",") if args.domains else None,
        )
        if args.max_clips:
    ...
    if args.train_domains:
        ...
        keep = {DOMAIN_IDS[d.strip()] for d in args.train_domains.split(",") if d.strip()}
        m = torch.tensor([int(x) in keep for x in D_tr])
        Z_tr, D_tr = Z_tr[m], D_tr[m]
        print(f"[transfer] train restricted to {args.train_domains}: {Z_tr.size(0)} clips; "
    ...
    if args.cache_only:
        ...
        print(f"[cache-only] saved {tuple(Z_tr.shape)} + val {tuple(Z_val.shape)} -> {args.out}")
        return

    wcfg = HJEPAConfig(
        latent_dim=embed_dim, hidden_dim=args.hidden,
    ...
    if args.ablation == "gru":
        ...
        kind = "hjepa"
    print(f"[model] {kind}/{args.ablation} {sum(p.numel() for p in model.parameters())/1e6:.2f}M")
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(0, Z_tr.size(0), bs):
            ...
            if kind in ("gru", "mamba"):
                ...
                z_hist, z_future = z_pool[:, :history], z_pool[:, history:history + horizon]
                pred = model(z_hist, d)
    ...
    with torch.no_grad():
        ...
        metrics["cos_persist"] = F.cosine_similarity(persist, z_future, dim=-1).mean().item()
        ...
        if is_dense and args.dense and kind == "hjepa":
            ...
            metrics["cos_model_dense"] = F.cosine_similarity(pred_d, zf_d, dim=-1).mean().item()
            metrics["mse_model_dense"] = (pred_d - zf_d).pow(2).mean().item()
        if kind == "hjepa" and args.ablation == "full":
            z_goal = z_future[:, -1]
            plan_a, plan_e = model.plan(z_hist, z_goal, D_val, n_samples=8, steps=horizon)
            metrics["plan_energy"] = plan_e.mean().item()
            metrics["plan_actions_shape"] = list(plan_a.shape)
    print(f"[rollout|val] cos  model={metrics['cos_model']:.3f}  persistence={metrics['cos_persist']:.3f}")
    if "cos_model_dense" in metrics:
        print(f"[rollout|val] cos  spacetime-dense={metrics['cos_model_dense']:.3f}")
    print(f"[rollout|val] mse  model={metrics['mse_model']:.4f}  persistence={metrics['mse_persist']:.4f}")

    ckpt = os.path.join(args.out, "endohjepa.pt")
    torch.save({
        "kind": kind, "ablation": args.ablation, "dense": args.dense,
        "model": model.state_dict(), "wcfg": wcfg.__dict__,
        "history": history, "horizon": horizon, "embed_dim": embed_dim,
        "seed": args.seed,
    }, ckpt)
    ...
    cache_pack = {"Z": _maybe_pool(Z).cpu(), "D": Doms.cpu(),
    ...
    if val_is_videolevel and split_va:
        try:
            ...
            n_va = Z_val.size(0)
            idx = domain_balanced_indices(ds_names.clips, n=min(n_va, len(ds_names.clips)), seed=1)
...
def build_argparser():
    ...
    ap.add_argument("--ablation", choices=["l1", "l1l2", "full", "gru", "mamba"], default="full")
    ap.add_argument("--dense", action="store_true",
                    help="predict unpooled spatiotemporal tokens (L1)")
    ap.add_argument("--predictor", choices=["spacetime", "persite"], default="spacetime",
                    help="L1 dense predictor: factorised space-time (v2) or legacy per-site")
    ap.add_argument("--vicreg", type=float, default=0.1,
                    help="weight of VICReg anti-collapse regulariser on dense predictions")
    ap.add_argument("--no-unc-weight", action="store_true",
    ...
    ap.add_argument("--l1-causal", action="store_true", default=True,
                    help="use autoregressive causal (GPT-style) L1 predictor (default, best)")
    ap.add_argument("--no-l1-causal", action="store_false", dest="l1_causal",
    ...
    ap.add_argument("--clip-len", type=int, default=0,
    ...
    ap.add_argument("--latents", default="", help="reuse a latents_cache.pt for fair ablations")
    ap.add_argument("--save-dense-cache", action="store_true",
