...
class VLM(nn.Module):
    def __init__(self, llm_id=LLM_ID, device="cuda"):
        ...
        self.llm = AutoModelForCausalLM.from_pretrained(llm_id, dtype=torch.float32).to(device)
        for p in self.llm.parameters():
            p.requires_grad_(False)
        self.llm.eval()
        d_llm = self.llm.config.hidden_size
        self.vision = CNNVisionEncoder(dim=256)
        self.projector = nn.Sequential(nn.Linear(256, d_llm), nn.GELU(), nn.Linear(d_llm, d_llm))
        self.device = device
    ...
    def forward(self, images, cap_ids, cap_mask):
        ...
        labels = torch.full((b, inp.size(1)), -100, dtype=torch.long, device=self.device)
        labels[:, pre:] = torch.where(cap_mask.bool(), cap_ids, torch.full_like(cap_ids, -100))
    ...
    def generate(self, images, max_new_tokens=40):
        ...
        attn = torch.ones(inp.size(0), inp.size(1), device=self.device)
        gen = self.llm.generate(inputs_embeds=inp, attention_mask=attn,
...
class PairDS(Dataset):
    def __init__(self, rows, tok, image_size=224, max_len=48):
        self.rows, self.tok, self.image_size, self.max_len = rows, tok, image_size, max_len
    ...
    def __getitem__(self, i):
        ...
        enc = self.tok(cap + self.tok.eos_token, truncation=True, max_length=self.max_len,
...
def train(args):
    ...
    print(f"[model] LLM={args.llm} (frozen); trainable={n_train:.1f}M (vision+projector)")
    ...
    for epoch in range(args.epochs):
        model.vision.train(); model.projector.train()
        run = 0.0
    ...
    torch.save({"vision": model.vision.state_dict(),
                "projector": model.projector.state_dict(), "llm": args.llm},
