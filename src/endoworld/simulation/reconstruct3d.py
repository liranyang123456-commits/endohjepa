...
def read_scared_intrinsics(calib_yaml: str) -> CameraIntrinsics:
    ...
    fs.release()
    return CameraIntrinsics(fx=float(m1[0, 0]), fy=float(m1[1, 1]),
                            cx=float(m1[0, 2]), cy=float(m1[1, 2]))


def load_scared_keyframe(keyframe_dir: str, min_depth_mm: float = 5.0,
    ...
    raw = cv2.imread(os.path.join(keyframe_dir, "left_depth_map.tiff"), cv2.IMREAD_UNCHANGED)
...
def reconstruct_scared_keyframe(keyframe_dir: str, out_ply: str,
                                max_depth_mm: float = 300.0) -> int:
...
def render_points(points, colors, intr: CameraIntrinsics, out_size, R=None, t=None,
                  point_radius: int = 1):
