...
class GaussianModel(nn.Module):
    ...
    def __init__(
        self, points: torch.Tensor, colors: torch.Tensor, init_scale: float = 1.5
    ):
        super().__init__()
        n = points.shape[0]
        self.means = nn.Parameter(points.clone())
        self.log_scale = nn.Parameter(
            torch.full((n, 1), float(torch.log(torch.tensor(init_scale))))
        )
    ...
    def render(self, cam: Camera, kernel: int = 5, near: float = 1.0):
        ...
        du = du.reshape(-1).float()
        dv = dv.reshape(-1).float()  # (O,)
        ...
        flat = (pix_v.clamp(0, cam.H - 1) * cam.W + pix_u.clamp(0, cam.W - 1)).reshape(
            -1
        )
        wf = w.reshape(-1)
...
def fit(
    ...
    for it in range(iters):
        ...
        if (it + 1) % log_every == 0 or it == 0:
            ...
            print(
                f"  [gs] iter {it + 1:4d} L1={loss.item():.4f} PSNR={psnr.item():.2f}dB"
            )
    return model
