...
def stir_endpoint_consistency(
    z_dense: torch.Tensor,
    pts_start: torch.Tensor,
    pts_end: torch.Tensor,
    image_size: int,
) -> torch.Tensor:
...
def depth_consistency(
    z_dense: torch.Tensor, depth: torch.Tensor, patch: int
) -> torch.Tensor:
    ...
    if h * w != n:
        return temporal_smoothness(z_dense)
    dep = F.interpolate(
        depth.reshape(b * t, 1, *depth.shape[-2:]),
        size=(h, w),
        mode="bilinear",
        align_corners=False,
    )
