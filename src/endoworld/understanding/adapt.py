...
def multi_horizon_baseline(Z: torch.Tensor, history: int) -> list[dict]:
    ...
    for h in (1, 4, 8, 16):
        ...
        rows.append({
            "horizon": h,
            "cos_linear": F.cosine_similarity(pred[:, :h], z_fut[:, :h], dim=-1).mean().item(),
...
def train_vjepa2_l1(args, device):
    ...
    ds = EndoClipDataset(
        ...
        split=None if args.smoke else args.split, return_meta=True,
    )
    if args.smoke:
    ...
    dl = DataLoader(ds, batch_size=max(1, args.batch_size // 4), shuffle=True,
    ...
    wcfg = HJEPAConfig(latent_dim=dim, hidden_dim=min(512, dim), n_heads=8,
                       n_layers=4, history=history, horizon=horizon,
                       spatial_keep=args.spatial_keep, ablation="l1")
    ...
    torch.save({"model": model.state_dict(), "wcfg": wcfg.__dict__,
                "history": history, "horizon": horizon, "kind": "hjepa",
                "ablation": "l1", "dense": True, "embed_dim": dim},
               os.path.join(args.out, "vjepa2_l1.pt"))
    torch.save({"Z": Z.cpu(), "D": D.cpu(), "dense": True},
...
def train_vjepa2_e2e(args, device):
    ...
    dl = DataLoader(ds, batch_size=1, shuffle=True, num_workers=args.workers,
                    collate_fn=collate_meta, drop_last=True)
    history, horizon = 4, 4
    wcfg = HJEPAConfig(latent_dim=dim, hidden_dim=min(512, dim), n_heads=8,
                       n_layers=2, history=history, horizon=horizon,
                       spatial_keep=args.spatial_keep, ablation="l1",
                       predictor="spacetime")
    model = EndoHJEPA(wcfg).to(device)
    params = list(model.parameters()) + enc.trainable_parameters()
    n_train = sum(p.numel() for p in enc.trainable_parameters()) / 1e6
    print(f"[e2e] encoder trainable {n_train:.1f}M + predictor {sum(p.numel() for p in model.parameters())/1e6:.1f}M")
    ...
    torch.save({"encoder": enc.model.state_dict(), "model": model.state_dict(),
                "wcfg": wcfg.__dict__, "unfreeze_last": args.unfreeze_last,
                "vjepa2_id": args.vjepa2_id, "kind": "hjepa", "ablation": "l1",
                "dense": True, "embed_dim": dim, "history": history, "horizon": horizon},
...
def train(args):
    ...
    ds = EndoClipDataset(
        args.manifest, clip_len=cfg.clip_len, stride=args.stride,
        image_size=cfg.image_size, exclude=["EndoVis2019_ROBUST-MIS"],
        split=split,
    )
    if args.smoke:
        if len(ds) == 0:
    ...
    print(f"[model] VJEPA L1-adapt {n_params:.1f}M  tokens={cfg.n_tokens}  endo_mask={args.endo_mask}")
    ...
    for epoch in range(args.epochs):
        ...
        for clip in dl:
            ...
            weights = None
            if args.endo_mask:
                weights = token_loss_weights(clip, cfg.tubelet_size, cfg.patch_size).to(device)
        ...
        if args.instrument_mask:
            ...
            print(f"[epoch {epoch}] avg_loss={avg:.4f} time={time.time()-t0:.1f}s  endovis_steps={extra}")
        else:
...
def _endovis_mask_steps(model, opt, device, cfg, args) -> int:
    ...
    for _, clip, inst in iter_endovis_clips(root, "train", cfg.clip_len, cfg.image_size, limit=4):
        clip = clip.unsqueeze(0).to(device).float()
        inst = inst.unsqueeze(0).to(device)
        weights = token_loss_weights(
            clip, cfg.tubelet_size, cfg.patch_size,
            instrument_mask=inst, instrument_boost=args.instrument_boost)
        loss = model(clip, token_weights=weights)
        opt.zero_grad(set_to_none=True)
        loss.backward()
        opt.step()
...
def build_argparser():
    ...
    ap.add_argument("--smooth-reg", type=float, default=0.05,
                    help="temporal smoothness on dense tokens")
    ap.add_argument("--stir-reg", type=float, default=0.0,
                    help=">0: dump STIR endpoint chamfer after V-JEPA2 adapt")
    ap.add_argument("--instrument-mask", action="store_true",
