...
def _fit_probe(head, feats, target, task, device, seed, steps=400, lr=0.02):
...
def _run_fold(feat, yphase, yinstr, vids, device, test_videos, seeds):
    ...
    for seed in seeds:
        ph = _fit_probe(AttentionPhaseProbe(feat.size(-1)),
        ...
        mil = _fit_probe(MILInstrumentProbe(feat.size(-1)),
