...
def _reach(model, z, D, history, horizon, n_samples=32):
    ...
    with torch.no_grad():
        ...
        plan_a, plan_e = model.plan(
            z_hist, z_goal, D, n_samples=n_samples, steps=horizon
        )
...
def main():
    ...
    if args.latents and Path(args.latents).is_file():
        ...
        idx = domain_balanced_indices(
            ds.clips, n=min(args.max_val, len(ds.clips)), seed=1
        )
        ds.clips = [ds.clips[i] for i in idx]
        dl = DataLoader(
            ds, batch_size=8, shuffle=False, num_workers=0, collate_fn=collate_meta
        )
