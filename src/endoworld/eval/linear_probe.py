...
def build_clips(images_dir, labels_dir, clip_len, stride, image_size, limit, max_clip_stride):
    ...
    for names in clips:
        frames = []
        for n in names:
            img = Image.open(os.path.join(images_dir, n)).convert("RGB").resize((image_size, image_size))
...
def run_probe(build_kwargs, encoders, args, device, batch=16):
    print(f"[probe] building clips (clip_len={build_kwargs['clip_len']}, "
          f"img={build_kwargs['image_size']}, stride={build_kwargs['stride']}) ...")
    X, Y, vids = build_clips(args.images, args.labels, build_kwargs["clip_len"],
                       build_kwargs["stride"], build_kwargs["image_size"],
                       args.limit, build_kwargs["stride"])
    print(f"[probe] {len(X)} clips / {len(set(vids))} videos; class positives: "
...
def main():
    ...
    ap.add_argument("--encoder", choices=["scratch", "vjepa2", "all"], default="scratch")
    ...
    if args.encoder in ("scratch", "all"):
        ...
        run_probe(
            {"clip_len": cfg.clip_len, "image_size": cfg.image_size, "stride": args.clip_stride},
            {"scratch-pretrained": pre, "scratch-random": rnd}, args, device, batch=args.batch)
    ...
    if args.encoder in ("vjepa2", "all"):
        ...
        run_probe(
            ...
            {"vjepa2-official": enc}, args, device, batch=args.vjepa2_batch)
