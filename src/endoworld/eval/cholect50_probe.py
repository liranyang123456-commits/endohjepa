...
from endoworld.data.cholect50 import (
    INSTR_NAMES, N_INSTR, N_PHASE, PHASE_NAMES, CHALLENGE_TEST_VIDS,
    CHOLECT50_CV_FOLDS,
    list_videos, load_video_labels, video_frames_dir,
)
...
def build_clips(root, clip_len, stride, image_size, max_per_video):
    ...
    for vid in list_videos():
        ...
        if n < clip_len:
            continue
        # subsample clip starts evenly across the video
        starts = np.linspace(0, n - clip_len, min(max_per_video, max(1, n // clip_len))).astype(int)
        for s in starts:
            idxs = [min(s + i * stride, n - 1) for i in range(clip_len)]
            imgs = []
            ok = True
            for i in idxs:
                try:
    ...
    return (torch.from_numpy(np.stack(X)), torch.tensor(Yphase, dtype=torch.long),
            torch.from_numpy(np.stack(Yinstr)), vids)
...
def probe_phase(feat, yphase, vids, device, official=False, seeds=(0, 1, 2),
                test_videos=None):
    ...
    for seed in seeds:
        ...
        for c in range(N_PHASE):
            m = yphase[te_t] == c
            if m.any():
                per_accum.setdefault(PHASE_NAMES[c], []).append(float((pred[m] == c).float().mean()))
    accs = np.array(accs)
    per = {k: {"mean": float(np.mean(v)), "std": float(np.std(v))} for k, v in per_accum.items()}
    return {"acc": float(accs.mean()), "acc_std": float(accs.std()), "accs": accs.tolist(),
            "per_class_acc": per, "n_train": len(tr), "n_test": len(te), "n_seeds": len(seeds)}


def probe_instrument(feat, yinstr, vids, device, official=False, seeds=(0, 1, 2),
                     test_videos=None):
    ...
    for seed in seeds:
        ...
        opt = torch.optim.Adam(head.parameters(), lr=0.05, weight_decay=1e-4)
        for _ in range(300):
            loss = F.binary_cross_entropy_with_logits(head(fz[tr_t].to(device)), yinstr[tr_t].to(device))
            opt.zero_grad(); loss.backward(); opt.step()
        with torch.no_grad():
    ...
    maps = np.array(maps)
    return {"mAP": float(maps.mean()), "mAP_std": float(maps.std()), "mAPs": maps.tolist(),
            "n_seeds": len(seeds)}
...
def main():
    ...
    ap.add_argument("--adapted-ckpt", default="outputs/vjepa2_adapted/vjepa2_adapted.pt")
    ...
    ap.add_argument("--encoders", default="vjepa2-frozen",
    ...
    if args.compare and "vjepa2-adapted" not in encoders and Path(args.adapted_ckpt).is_file():
        ad, _, _, _ = load_adapted_vjepa2(args.adapted_ckpt, device)
        encoders["vjepa2-adapted"] = ad

    # build clips once at a shared size; each encoder resizes internally
    X, Yphase, Yinstr, vids = build_clips(args.root, args.clip_len, args.stride,
                                          256, args.max_per_video)
    if X is None:
        print("[cholect50] no clips built; check frames extracted")
        return
    print(f"[cholect50] {len(X)} clips / {len(set(vids))} videos")

    report = {"paper": "Endo-HJEPA", "video_level": True, "not_ablation_planning": True,
              "official_split": bool(args.official_split),
              "crossval": bool(args.crossval),
              "test_videos": (CHOLECT50_CV_FOLDS if args.crossval else
                              CHALLENGE_TEST_VIDS if args.official_split else "hash"),
              "n_clips": len(X), "n_videos": len(set(vids)), "results": {}}
    for tag, model in encoders.items():
        ...
        if args.crossval:
            ...
            print(f"[{tag}] CV phase acc={cv['phase']['acc']:.3f}  "
            ...
            print(f"[{tag}] phase acc={ph['acc']:.3f}  instrument mAP={inst['mAP']:.3f}")
    if len(encoders) == 2 and not args.crossval:
        report["adaptation_gain_phase"] = (report["results"]["vjepa2-adapted"]["phase"]["acc"]
                                          - report["results"]["vjepa2-frozen"]["phase"]["acc"])
        report["adaptation_gain_instrument"] = (report["results"]["vjepa2-adapted"]["instrument"]["mAP"]
