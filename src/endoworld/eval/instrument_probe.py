...
from endoworld.data.endovis_masks import list_endovis_pairs, load_class_map, load_mask, presence_vector
from endoworld.data.splits import assign_split
from endoworld.eval.linear_probe import average_precision, extract_features, load_encoder
...
def probe_one(model, X, Y, vids, device, batch):
    ...
    for i, v in enumerate(vids):
        (tr if assign_split(f"endovis::{v}", train=0.8, val=0.0) == "train" else te).append(i)
    if not te:
...
def main():
    ...
    if args.compare:
        encoders = {}
        fr, clip_len, image_size, _ = load_any_encoder("vjepa2", device, args.vjepa2_id, args.vjepa)
        ...
        model, clip_len, image_size, _ = load_any_encoder("vjepa2", device, args.vjepa2_id, args.vjepa)
    ...
    for tag, model in encoders.items():
        ...
        results[tag] = {"mAP": mAP, "per_class": dict(zip(names, aps)),
                        "n_train": ntr, "n_test": nte}
        print(f"[{tag:16s}] video-level mAP = {mAP:.3f}")
    report = {"video_level": True, "paper": "Endo-HJEPA", "not_cholecseg8k_0992": True,
              "main_table_ok": True, "results": results}
    if "vjepa2-frozen" in results and "vjepa2-adapted" in results:
        report["adaptation_gain"] = results["vjepa2-adapted"]["mAP"] - results["vjepa2-frozen"]["mAP"]
    Path(args.out).parent.mkdir(parents=True, exist_ok=True)
    Path(args.out).write_text(__import__("json").dumps(report, indent=2), encoding="utf-8")
