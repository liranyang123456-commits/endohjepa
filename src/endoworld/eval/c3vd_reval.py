...
def corrected_c3vd_sequence(
    seq: PhysicalSequence,
    c3vd_root: Path,
    stride: int,
    ...
    matches = [
        pose for pose in pose_files if pose.parent.resolve() == sequence_path.resolve()
    ]
    if len(matches) != 1:
        raise RuntimeError(
...
def main():
    ...
    parser.add_argument(
        "--checkpoint", default="outputs/continuous_actions_v2/continuous_dynamics.pt"
    )
    parser.add_argument("--c3vd", default="datasets/C3VD")
    parser.add_argument("--stride", type=int, default=2)
    parser.add_argument("--lookback-tubelets", type=int, default=8)
    parser.add_argument(
        "--out", default="outputs/continuous_actions_v2/c3vd_corrected_eval.json"
    )
