...
class MILHead(torch.nn.Module):
    ...
    def forward(self, tokens):  # (B, N, D)
        ...
        weights = torch.softmax(
            tokens @ self.phase_query / tokens.size(-1) ** 0.5, dim=1
        )
...
def evaluate(enc, head, X, yphase, yinstr, idx, device, batch):
    ...
    mAP = float(
        np.nanmean([average_precision(scores[:, c], yte[:, c]) for c in range(N_INSTR)])
    )
    return acc, mAP
...
def main():
    ...
    parser.add_argument(
        "--out", default="outputs/vjepa2_adapted/cholect50_finetune_mil_crossval.json"
    )
    ...
    for fold, test_videos in CHOLECT50_CV_FOLDS.items():
        ...
        for epoch in range(args.epochs):
            ...
            for i in range(0, len(tr), args.batch):
                ...
                nb += 1
            acc, mAP = evaluate(
                enc, head, X, Yphase, Yinstr, torch.tensor(te), device, args.batch
            )
            print(
                f"[{fold} epoch {epoch + 1}] loss={running / max(nb, 1):.4f} "
                f"test acc={acc:.3f} mAP={mAP:.3f}",
                flush=True,
            )
